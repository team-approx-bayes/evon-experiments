...
@torch.compile
def correct_orthogonal_bf16(X: torch.Tensor, n_iters: int = 2):
    """Improve orthogonality of a near-orthogonal bf16 matrix via Newton-Schulz."""
    buf = torch.empty_like(X)
    A2 = torch.empty_like(X)
    for _ in range(n_iters):
        torch.mm(X.T, X, out=A2)  # A2 = X^T @ X
        A2.mul_(-0.5)  # A2 = -0.5 * X^T @ X
        A2.diagonal().add_(1.5)  # A2 = 1.5*I - 0.5*X^T@X
        torch.mm(X, A2, out=buf)  # X = X @ (1.5*I - 0.5*X^T@X)
        X, buf = buf, X  # swap, no copy
    return X
...
class SOAP(optim.Optimizer):
    ...
    def __init__(
        ...
        self.improve_orth = improve_orth
        if self.improve_orth:
            print("improving orthogonality")
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # 2d case: state['Q'][0].T @ exp_avg @ state['Q'][1]
                ...
                # 2d case: state['Q'][0] @ (exp_avg_projected / denom) @ state['Q'][1].T
    ...
    def project(self, grad, state, merge_dims=False, max_precond_dim=10000):
        ...
        for mat in state["Q"]:
            if len(mat) > 0:
                mat_to_use = mat if mat.dtype == grad.dtype else mat.to(grad.dtype)
    ...
    def get_orthogonal_matrix(self, mat):
        ...
        # print('dtype in soap', m.data.dtype)
        ...
        for m in matrix:
            ...
            if not float_data:
                ...
                if self.improve_orth:
                    Q = correct_orthogonal_bf16(Q, n_iters=2)
            final.append(Q)
        return final
